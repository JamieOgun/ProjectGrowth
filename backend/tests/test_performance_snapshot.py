...
from growth_op_agent.analytics.performance import PerformanceAnalytics, latest_snapshot_path


def test_weekly_snapshot_uses_date_filename_and_includes_top_10_with_metadata(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "latest_summary",
        lambda self: SimpleNamespace(model_dump=lambda mode: {"current": {"followers": 123}}),
    )
    ...
    history_path.write_text(json.dumps([
        {
            ...
            "referenced_tweets": [{"id": f"ref-{i}", "type": "replied_to", "text": "Context"}],
            "media": [{"media_key": f"media-{i}", "type": "photo", "url": "https://example.com/x.jpg"}],
            "likes": i,
            "retweets": 0,
            "replies": 0,
            "impressions": 100,
    ...
    assert (snapshot_dir / f"weekly_snapshot_{datetime.now(timezone.utc):%d-%m-%Y}.json").exists()
